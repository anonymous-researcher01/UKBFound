...
if __name__ == "__main__":
    ...
    image_X = args.Xtype
    loc = args.string
    Xdata, _,lab = load(image_X, category,xloc=f'../../results/xblock/priority/{hyperp}/')
    print(
    ...
    *_, trainindex, valindex, testindex = loadindex(image_X)
    ...
    net = pheNN(shape_data, shape_label, NNdepth, NNshape)
    ...
    try:
        ...
        pass
    np.save(f"./{loc}pri/{category}{modelchar(model)}{image_X}_{hyperp}", aucresult)
    np.save(f"./{loc}pri/{category}{modelchar(model)}{image_X}_{hyperp}out", out)
    np.save(f"./{loc}pri/{image_X}lab", labels)
    torch.save(nnnet, f"./{loc}pri/{category}{modelchar(model)}{image_X}_{hyperp}model")
    print("complete")
