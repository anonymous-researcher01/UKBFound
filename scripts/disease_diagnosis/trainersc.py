...
if __name__ == "__main__":
    ...
    lab=lab[:,cats[key]]
    print(
        "model", key, "cat", category, len(Xdata), "params", imgimp, "cat", key
    )
    ...
    *_, trainindex, valindex, testindex = loadindex(image_X)
    ...
    try:
        ...
        pass
    np.save(
        f"./{loc}sc/{category}{key}_{imgimp}", aucresult
    )
    '''np.save(f"./{loc}sc/{category}{modelchar(model)}{0}{key}_{hyperp}out", out)
    np.save(
        f"./{loc}sc/{category}{modelchar(model)}{0}{key}_{hyperp}lab", labels
    )'''
    torch.save(
        nnnet, f"./{loc}sc/{category}{key}_{imgimp}model"
    )
    print("complete")
