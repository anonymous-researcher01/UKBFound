...
if __name__ == "__main__":
    ...
    loc = args.string
    print(loc)
    loc,position=loc.split('_')
    Xdata, _,lab = load(image_X, category,fullimgspec=int(position))
    print(
    ...
    *_, trainindex, valindex, testindex = loadindex(image_X)
    ...
    try:
        os.mkdir(f"./{loc}special")
    except:
        pass
    np.save(f"./{loc}special/{category}{position}", aucresult)
    ...
    #np.save(f"./{loc}special/{image_X}lab", labels)
    torch.save(nnnet, f"./{loc}special/{category}{position}model")
    print("complete")
