...
if __name__ == "__main__":
    ...
    epoch = 1000
    learning_rate = 0.005
    X, Y, E = load(image_X, category,xloc=f'../../results/xblock/priority/{hyperp}/')
    index = np.isnan(E)
    ...
    *_, trainindex, valindex, testindex = loadindex(imageX)
    ...
    try:
        os.mkdir(f"./{loc}survpri")
    except:
        pass
    np.save(f"./{loc}survpri/{category}{modelchar(model)}{image_X}_{hyperp}", cindex)
    np.save(f"./{loc}survpri/{category}{modelchar(model)}{imageX}_{hyperp}out", out)
    np.save(f"./{loc}survpri/{imageX}_{hyperp}lab", [ytest, etest])
    torch.save(nnet, f"./{loc}survpri/{category}{modelchar(model)}{imageX}_{hyperp}model")
    print("complete")
