...
if __name__ == "__main__":
    ...
    *_, trainindex, valindex, testindex = loadindex(imageX)
    ...
    try:
        os.mkdir(f"./{loc}survsc")
    except:
        pass
    np.save(f"./{loc}survsc/{category}{key}", cindex)
    np.save(f"./{loc}survsc/{category}{key}out", out)
    np.save(f"./{loc}survsc/{category}{key}lab", [ytest,etest])
    torch.save(nnet, f"./{loc}survsc/{category}{key}model")
    print("complete")
