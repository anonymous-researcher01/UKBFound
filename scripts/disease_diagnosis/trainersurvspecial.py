...
if __name__ == "__main__":
    ...
    epoch = 100
    learning_rate = 0.005
    X, Y, E = load(image_X=imageX, category=category, only=only,xloc=position)
    ...
    *_, trainindex, valindex, testindex = loadindex(imageX)
    ...
    try:
        ...
        pass
    np.save(f"./{loc}special/surv{category}{position.split('/')[-2]}", cindex)
    ...
    torch.save(nnet, f"./{loc}special/surv{category}{position.split('/')[-2]}model")
    print("complete")
