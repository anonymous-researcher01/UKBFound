library(tidyverse)
library(data.table)
ind_train <- read_csv("../../results/Preprocess/index/train.csv") %>% pull(train) + 1
ind_val <- read_csv("../../results/Preprocess/index/val.csv") %>% pull(val) + 1
ind_test <- read_csv("../../results/Preprocess/index/test.csv") %>% pull(test) + 1
